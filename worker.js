...
self.addEventListener('message', event => {
  if (event.data.levelId) levelID = event.data.levelId;
});
...
self.addEventListener("fetch", (event) => {
  ...
  if (levelID < 0) {
    if (url.pathname.includes("1.txt")) {
      event.respondWith(fetch(`/game/assets/levels/${levelID}.txt`));
      return;
    }
    if (url.pathname.includes("StereoMadness.mp3")) {
      event.respondWith(fetch(`/game/assets/music/${levelID}.mp3`));
      return;
    }
  }
  ...
  if (levelID >= 0) {
    ...
    if (url.pathname.includes("StereoMadness.mp3")) {
      event.respondWith(fetch(`https://getlevelsong.lasokar.workers.dev?id=${levelID}`));
      return;
    }
  }
});
